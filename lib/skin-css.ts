// Skin CSS strings keyed by skin name.
// Extracted from styles/skins/all-skins.css so the export pipeline
// can include the active skin without reading the stylesheet at runtime.

export const SKIN_CSS: Record<string, string> = {
  "modern-saas": `[data-skin="modern-saas"] {
  --background: 0 0% 100%;
  --foreground: 222 47% 11%;
  --card: 0 0% 100%;
  --card-foreground: 222 47% 11%;
  --popover: 0 0% 100%;
  --popover-foreground: 222 47% 11%;
  --primary: 221 83% 53%;
  --primary-foreground: 210 40% 98%;
  --secondary: 210 40% 96%;
  --secondary-foreground: 222 47% 11%;
  --muted: 210 40% 96%;
  --muted-foreground: 215 16% 47%;
  --accent: 210 40% 96%;
  --accent-foreground: 222 47% 11%;
  --destructive: 0 84% 60%;
  --destructive-foreground: 210 40% 98%;
  --success: 142 76% 36%;
  --success-foreground: 210 40% 98%;
  --warning: 38 92% 50%;
  --warning-foreground: 222 47% 11%;
  --border: 214 32% 87%;
  --input: 214 32% 87%;
  --ring: 221 83% 53%;
  --radius: 0.5rem;
  --sidebar-width: 16rem;

  --bg: var(--background);
  --fg: var(--foreground);
  --primary-fg: var(--primary-foreground);
  --muted-fg: var(--muted-foreground);
  --card-fg: var(--card-foreground);
  --destructive-fg: var(--destructive-foreground);
  --accent-fg: var(--accent-foreground);
}`,

  "warm-friendly": `[data-skin="warm-friendly"] {
  --background: 30 25% 99%;
  --foreground: 20 14% 12%;
  --card: 30 20% 100%;
  --card-foreground: 20 14% 12%;
  --popover: 30 20% 100%;
  --popover-foreground: 20 14% 12%;
  --primary: 38 92% 50%;
  --primary-foreground: 20 14% 12%;
  --secondary: 30 20% 94%;
  --secondary-foreground: 20 14% 12%;
  --muted: 30 15% 94%;
  --muted-foreground: 20 10% 44%;
  --accent: 30 20% 94%;
  --accent-foreground: 20 14% 12%;
  --destructive: 0 84% 60%;
  --destructive-foreground: 0 0% 100%;
  --success: 142 76% 36%;
  --success-foreground: 0 0% 100%;
  --warning: 25 95% 53%;
  --warning-foreground: 20 14% 12%;
  --border: 30 18% 82%;
  --input: 30 18% 82%;
  --ring: 38 92% 50%;
  --radius: 0.75rem;
  --sidebar-width: 16rem;

  --bg: var(--background);
  --fg: var(--foreground);
  --primary-fg: var(--primary-foreground);
  --muted-fg: var(--muted-foreground);
  --card-fg: var(--card-foreground);
  --destructive-fg: var(--destructive-foreground);
  --accent-fg: var(--accent-foreground);
}`,

  "dark-technical": `[data-skin="dark-technical"] {
  --background: 222 47% 7%;
  --foreground: 150 20% 92%;
  --card: 222 47% 9%;
  --card-foreground: 150 20% 92%;
  --popover: 222 47% 9%;
  --popover-foreground: 150 20% 92%;
  --primary: 142 76% 36%;
  --primary-foreground: 0 0% 100%;
  --secondary: 220 30% 14%;
  --secondary-foreground: 150 20% 92%;
  --muted: 220 30% 14%;
  --muted-foreground: 215 15% 55%;
  --accent: 220 30% 14%;
  --accent-foreground: 150 20% 92%;
  --destructive: 0 72% 51%;
  --destructive-foreground: 0 0% 100%;
  --success: 142 76% 36%;
  --success-foreground: 0 0% 100%;
  --warning: 38 92% 50%;
  --warning-foreground: 222 47% 7%;
  --border: 220 20% 20%;
  --input: 220 20% 20%;
  --ring: 142 76% 36%;
  --radius: 0.25rem;
  --sidebar-width: 16rem;

  --bg: var(--background);
  --fg: var(--foreground);
  --primary-fg: var(--primary-foreground);
  --muted-fg: var(--muted-foreground);
  --card-fg: var(--card-foreground);
  --destructive-fg: var(--destructive-foreground);
  --accent-fg: var(--accent-foreground);
}`,
};
