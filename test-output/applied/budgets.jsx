...
export default function Budgets() {
  ...
  const budgets = [
    { category: "Groceries", allocated: 600, spent: 287.45, transactions: 12, color: "hsl(152 55% 40%)", weeklySpending: [120, 145, 98, 132, 110, 156, 142, 128] },
    { category: "Gas", allocated: 200, spent: 145.30, transactions: 8, color: "hsl(45 90% 50%)", weeklySpending: [45, 52, 38, 48, 42, 55, 50, 48] },
    { category: "Entertainment", allocated: 150, spent: 87.50, transactions: 6, color: h("--primary"), weeklySpending: [25, 18, 32, 28, 22, 35, 30, 25] },
    { category: "Dining Out", allocated: 300, spent: 203.20, transactions: 15, color: "hsl(280 65% 55%)", weeklySpending: [65, 78, 55, 72, 68, 85, 75, 70] },
    { category: "Shopping", allocated: 400, spent: 456.78, transactions: 9, color: "hsl(15 75% 55%)", weeklySpending: [95, 110, 88, 105, 98, 125, 115, 105] },
    { category: "Utilities", allocated: 250, spent: 142.33, transactions: 3, color: "hsl(200 85% 45%)", weeklySpending: [58, 62, 55, 60, 57, 65, 62, 60] },
    { category: "Transportation", allocated: 180, spent: 95.60, transactions: 4, color: "hsl(120 45% 50%)", weeklySpending: [38, 42, 35, 40, 37, 45, 42, 40] },
    { category: "Healthcare", allocated: 200, spent: 45.00, transactions: 2, color: "hsl(340 75% 55%)", weeklySpending: [15, 18, 12, 16, 14, 20, 18, 16] }
  ];
          ...
          <div className="rounded-xl border" style={{ borderColor: ha("--border", 0.5) }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1px", background: ha("--border", 0.3) }}>
              {budgets.map((budget, i) => {
                ...
                const status = {
                  color: isOverBudget ? h("--destructive") : isNearLimit ? "hsl(45 90% 50%)" : budget.color
                };

                // Calculate sparkline coordinates
                const minSpending = Math.min(...budget.weeklySpending);
                const maxSpending = Math.max(...budget.weeklySpending, budget.allocated / 4);
                const range = maxSpending - minSpending || 1;
                
                const points = budget.weeklySpending.map((value, index) => {
                  const x = (index / (budget.weeklySpending.length - 1)) * 200;
                  const y = 32 - ((value - minSpending) / range) * 28;
                  return `${x},${y}`;
                }).join(" ");

                const budgetLineY = 32 - ((budget.allocated / 4 - minSpending) / range) * 28;
                  ...
                  <div key={budget.category} className="p-5 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300" 
                    ...
                    <div className="pt-2">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs" style={{ color: h("--muted-fg") }}>Weekly trend</span>
                        <span className="text-xs" style={{ color: h("--muted-fg") }}>{budget.weeklySpending.length} weeks</span>
                      </div>
                      <div className="h-8 relative">
                        <svg viewBox="0 0 200 32" className="w-full h-full">
                          <polygon points={`0,32 ${points} 200,32`} fill={status.color} opacity="0.1" />
                          <polyline points={points} fill="none" stroke={status.color} strokeWidth="1.5" opacity="0.8" />
                          <line x1="0" y1={budgetLineY} x2="200" y2={budgetLineY} stroke={ha("--muted-fg", 0.4)} strokeDasharray="2,2" strokeWidth="1" />
                        </svg>
                      </div>
                    </div>
